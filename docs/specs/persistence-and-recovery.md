...
Project content uses canonical schema-31 JSON. A private Cloud Project is the
...
always writes schema 31. Older schemas are rejected outside the rolling
current-and-previous compatibility window.

Recovery state is a non-authoritative browser safety copy. It may restore a
complete schema-31 Project or a schema-30 record that validates after the
